...
| **Compte utilisateur minimal** : **inscription / connexion email + mot de passe** (hash + sel, etc.)                                                | Quasi ( gerer le sel du mdp) | Even + Cloe         | **Requis III** même si OAuth en module                                                                                                                                                                    |
...
| **Interaction utilisateurs** (chat minimal, profil visible, amis add/remove + liste) | Major | 2   | Quasi (probleme sur la partie bloquer / supprimer qu'il faut ajouter) | Vyke, Fey + Even     | **2026-04** : app **chat** Django + WS, drawer React, contacts/conv ; **profil** éditable + avatar + amis + classement ; API friends. Le **chat avancé** (invite partie, etc.) reste **Minor bonus** si revendiqué à part. |
...
| Module (PDF)                                                                                     | Type  | Pts | Statut                                                    | Owner                           | Notes                                                                                                                                                |
| ------------------------------------------------------------------------------------------------ | ----- | --- | --------------------------------------------------------- | ------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| Gestion **standard** (profil éditable, avatar + défaut, **amis** + statut en ligne, page profil) | Major | 2   | Fait (check au cas ou l'avatar par defaut)                | Vyke, Fey + Even                | **2026-04** : endpoints profil, upload avatar, friendships, leaderboard mock/API ; UI profil refonte.                                                |
| **OAuth 2.0** (42, Google, GitHub…)                                                              | Minor | 1   | Fait                                                      | Even + Vyke, Fey                | **42** : niveau Intra exposé côté user ; autres providers selon démo.                                                                                |
| **Stats & historique de parties** (nécessite jeu fonctionnel)                                    | Minor | 1   | En cours (il manque match history, link le front au back) | Mileum (Théo), Even + Vyke, Fey | **Front** : page `/statistics` (stats perso, graphiques Recharts, thème coalition, mock JSON) ; **reste** : persistance / API parties réelles (Even) |
...
|                                                                                       |       |     |                                                                |                    |                                                                                                                                   |
| **Mode spectateur**                                                                   | Minor | 1   | Pas fait                                                       | Even               |                                                                                                                                   |
...
| Module (PDF)                                                                   | Type  | Pts | Statut                                                           | Owner                 | Notes                                                                                                       |
| ------------------------------------------------------------------------------ | ----- | --- | ---------------------------------------------------------------- | --------------------- | ----------------------------------------------------------------------------------------------------------- |
| **ELK** (ES + Logstash + Kibana, rétention, sécurisation)                      | Major | 2   | ?                                                                | Mileum (Théo) + Cloe  | Finaliser rétention / accès                                                                                 |
| **Prometheus + Grafana** (metrics, dashboards, **alerting**, sécurité Grafana) | Major | 2   | ? (a voir si on separe la partie server admin et le server user) | Cloé                  | **Alerting** à finaliser                                                                                    |
| **Microservices** (découplage, interfaces claires, REST ou files, **SRP**)     | Major | 2   | ?(messages queue ??)                                             | Cloe + Mileum         | Argumenter frontières de services                                                                           |
| **Health checks**, page de statut, **sauvegardes auto** + **PRA / reprise**    | Minor | 1   | ?                                                                | Even, Cloé (+ Mileum) | EN : *Health check and status page system with automated backups and disaster recovery procedures.* — bonus |
...
|                                                                            |       |     |          |               |                                                                                         |
| **RGPD** (accès données, suppression, export lisible, emails confirmation) | Minor | 1   | Pas fait | Even          | Écrans : Vyke, Fey                                                                      |
