...
| **Git** : commits de tous, messages clairs, répartition du travail                                                                                  | Quasi    | Équipe              | Branche `vbonnard` : gros lot **2026-04-05** (feat chat / profil / CSS) + merges `main`.                                                                                                                                                                                                           |
...
| Front **clair, responsive** ; solution **CSS** (framework ou autre)                                                                                 | Quasi    | Vyke, Fey           | Bottom nav mobile, panneau stats sous le plateau en étroit, shell commun.                                                                                                                                 |
...
| **WebSockets** (temps réel, déconnexions, diffusion efficace)                        | Major | 2   | En cours       | Even, Fey            | Serveur : Even ; client : Vyke, Fey                                                                                                                                                                           |
| **Interaction utilisateurs** (chat minimal, profil visible, amis add/remove + liste) | Major | 2   | Quasi          | Vyke, Fey + Even     | **2026-04** : app **chat** Django + WS, drawer React, contacts/conv ; **profil** éditable + avatar + amis + classement ; API friends. Le **chat avancé** (invite partie, etc.) reste **Minor bonus** si revendiqué à part. |
| ORM                                                                                  | Minor | 1   | Quasi          | Even + Cloe + Mileum | Django ORM                                                                                                                                                                                                    |
|                                                                                      |       |     |                |                      |                                                                                                                                                                                                               |
...
| Module (PDF)                                                                                     | Type  | Pts | Statut         | Owner                           | Notes                                                                                                                                                |
| ------------------------------------------------------------------------------------------------ | ----- | --- | -------------- | ------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| Gestion **standard** (profil éditable, avatar + défaut, **amis** + statut en ligne, page profil) | Major | 2   | Quasi          | Vyke, Fey + Even                | **2026-04** : endpoints profil, upload avatar, friendships, leaderboard mock/API ; UI profil refonte.                                                                                                                                    |
| **OAuth 2.0** (42, Google, GitHub…)                                                              | Minor | 1   | Quasi          | Even + Vyke, Fey                | **42** : niveau Intra exposé côté user ; autres providers selon démo.                                                                                                                   |
...
| **2FA** complet                                                                                  | Minor | 1   | Quasi          | Even                            | **Front** : toggle placeholder retiré des Paramètres tant que pas de backend ; réintégrer UI quand TOTP/API prêts.                                   |
...
| **Remote players** (2 machines, latence, déco, **reconnexion**)                           | Major | 2   | Quasi           | Even + Vyke, Fey   | **Cœur du gap** vs hot-seat / mock                                                                                                                                   |
...
| Module (PDF)                                                                   | Type  | Pts | Statut         | Owner                     | Notes                                                                                                       |
| ------------------------------------------------------------------------------ | ----- | --- | -------------- | ------------------------- | ----------------------------------------------------------------------------------------------------------- |
| **ELK** (ES + Logstash + Kibana, rétention, sécurisation)                      | Major | 2   | Quasi          | Mileum (Théo)             | Finaliser rétention / accès                                                                                 |
...
| Module (PDF)                                                               | Type  | Pts | Statut   | Owner                     | Notes                                                                                   |
| -------------------------------------------------------------------------- | ----- | --- | -------- | ------------------------- | --------------------------------------------------------------------------------------- |
| Dashboard analytique avancé (charts, temps réel, export, filtres)          | Major | 2   | En cours | Mileum (Théo)             | **Front** : écran stats perso avec graphiques (complémentaire au dashboard global IV.8) |
|                                                                            |       |     |          |                           |                                                                                         |
| **RGPD** (accès données, suppression, export lisible, emails confirmation) | Minor | 1   | À faire  | Mileum (Théo), Even + Fey | Écrans : Vyke, Fey                                                                      |
...
| --------------------------------------------------------------------------------------------------------------- | ----- | --- | ------------ | ----- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Audio** — expérience sonore globale (BGM, SFX, préférences partagées jeu + home, polish fin de partie / menu) | Minor | 1   | Quasi, bonus | Vyke  | BGM home (`Beth's Story.m4a`) hors `/game/`* ; BGM partie (`Theme_of_game.wav`) au 1er coup ; chrono blanc figé avant 1er coup (UI) ; README `public/sounds/` + racine à jour. |
...
| 01/04/2026 | **UI / jeu** : shell **responsive** (bottom nav, layout partie), onglets stats isolés ; **audio** : nouveaux assets BGM, musique **home** par route, BGM **partie** au 1<sup>er</sup> coup, correctifs cycle de vie BGM accueil, audibilité, **budget hauteur** plateau ; **échiquier** : coordonnées, mobile ; **chess** : fantôme de coup + **roque** (plateau pré-mouvement), seed tuiles, son refus coup illégal ; **DevOps** : Postgres prêt avant migrations / worker Celery ; **nginx** `conf.d` ; **Makefile**. |
