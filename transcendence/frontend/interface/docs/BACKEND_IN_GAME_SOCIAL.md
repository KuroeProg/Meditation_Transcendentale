...
> Ces actions ne constituent pas une suppression de compte RGPD. Voir section 9.7–9.8 pour les endpoints serveur d’export et de suppression.
...
### 9.4bis Invitations partie — garde « déjà en partie »
...
| Méthode | Route | Rôle |
|---------|--------|------|
| `POST` | `/api/auth/me/delete-data` | Même logique qu’en `DELETE` ; corps JSON `{"confirm": true}` requis. Utilisé par l’UI (CSRF, contourne un `403` possible sur `DELETE` derrière un proxy / WAF). En-têtes : `X-CSRFToken` + cookie de session. |
| `DELETE` | `/api/auth/me/delete-data` | Anonymise le compte (PII : username → `deleted_<12hex>`, email, nom, bio, avatar, mot de passe, préférences). Supprime les messages **envoyés**, les **accusés de lecture** (`read_by`) pour ce compte, retire le compte des **participants** des conversations, supprime amitiés et invitations. Efface les clés Redis **présence** et **`active_game`** pour l’utilisateur. Les lignes `Game`/`Move` restent liées au compte anonymisé. Journalisation serveur : `rgpd_delete`. Répond `{ ok: true, message }` ou `{ error }`. |
...
**Côté client** (`Settings.jsx`) : section « Données personnelles (RGPD) » avec export JSON, lien vers `public/PRIVACY_RETENTION.md`, double confirmation de suppression (étape 0 → 1 → 2 `in-progress` → 3 `done`) et logout automatique après 2,5 s. Appel **POST** `+` `X-CSRFToken` pour la suppression.

### 9.8 RGPD — export portabilité

| Méthode | Route | Rôle |
|---------|--------|------|
| `GET` | `/api/auth/me/export-data` | Réponse **JSON** (téléchargement `Content-Disposition: attachment`) : profil (`to_public_dict`), `client_prefs`, jusqu’à 800 parties résumées, jusqu’à 500 invitations, conversations dont l’utilisateur est membre avec jusqu’à 4000 messages au total (`messages_truncated` si plafond atteint). Journalisation : `rgpd_export`. |

**Fichier public** : `PRIVACY_RETENTION.md` (servi par le frontend sous `/PRIVACY_RETENTION.md`) décrit les durées de conservation indicatives (session, Redis présence / partie active, invitations).

---

*Mis à jour — avril 2026 (invites garde partie active, chrono 2 demi-coups, RGPD export + suppression étendue, rétention documentée).*
