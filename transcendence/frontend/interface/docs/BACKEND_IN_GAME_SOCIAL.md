...
**Côté client** (`Settings.jsx`) : section « Données personnelles (RGPD) » avec double confirmation (étape 0 → 1 → 2 `in-progress` → 3 `done`) et logout automatique après 2,5 s. Appel **POST** `+` `X-CSRFToken` (pas `DELETE` seul) pour la compatibilité infra.
